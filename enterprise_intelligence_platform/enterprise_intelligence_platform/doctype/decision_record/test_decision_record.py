...
class TestDecisionRecord(FrappeTestCase):
	...
	def setUpClass(cls):
		...
		cls.operations_user = cls.make_user_with_role(
			"eip_decision_operations@example.com", "EIP Operations Manager"
		)
	...
	def test_owner_can_create_without_permission_bypass(self):
		frappe.set_user(self.owner_user)
		doc = self.make_decision_doc().insert()
		self.assertEqual(doc.doctype, "Decision Record")
	...
	def test_operations_manager_cannot_write_without_permission_bypass(self):
		...
		doc.reload()
		doc.business_decision_summary = "Ops manager edit attempt"
	...
	def test_empty_approval_state_is_normalized_to_draft(self):
		...
		doc_save = self.make_decision_doc().insert(ignore_permissions=True)
		doc_save.reload()
		...
		doc_save_ignore = self.make_decision_doc().insert(ignore_permissions=True)
		...
		doc_insert = self.make_decision_doc()
