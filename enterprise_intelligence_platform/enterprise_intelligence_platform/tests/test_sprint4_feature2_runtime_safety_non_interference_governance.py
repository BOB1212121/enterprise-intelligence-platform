...
from enterprise_intelligence_platform.sprint4_feature2_runtime_safety_non_interference_governance import (
	DEFER,
	GO,
	NO_GO,
...
class TestSprint4Feature2RuntimeSafetyNonInterferenceGovernance(FrappeTestCase):
	@classmethod
	def setUpClass(cls):
		super().setUpClass()
		...
		cls.owner_user = cls.make_user_with_role("s4f2_owner@example.com", "EIP Workflow Owner")
		cls.sponsor_user = cls.make_user_with_role("s4f2_sponsor@example.com", "EIP Executive Sponsor")
		cls.operations_user = cls.make_user_with_role("s4f2_operations@example.com", "EIP Operations Manager")
		cls.system_manager_user = cls.make_user_with_role("s4f2_system@example.com", "System Manager")
		cls.unprivileged_user = cls.make_base_user("s4f2_unprivileged@example.com")
		...
		return frappe.get_doc(
			{
				...
				"workflow_name": f"S4F2 Charter {random_string(8)}",
				"business_objective": "Sprint 4 Feature 2 runtime safety and non-interference governance.",
				"in_scope_definition": "Sprint 4 Feature 2 planning-only governance scope.",
		...
		return frappe.get_doc(
			{
				...
				"decision_title": f"S4F2 Decision {random_string(8)}",
				...
				"business_decision_summary": "Sprint 4 Feature 2 governance context.",
	...
	def make_evidence_package(
		...
		*,
		...
		if defer_blocker_type != "none" and defer_blocker_reference is None:
			defer_blocker_reference = "S4F2-DEP-001"

		package = {
			"evidence_package_id": f"S4F2-EVID-{random_string(8)}",
			"policy_version": "S4F2_POLICY_V1",
			"feature_scope_declaration": "Sprint 4 Feature 2 runtime safety and non-interference governance only",
			...
			"regression_non_interference_declaration": "Covers frozen baseline and approved Feature 1, Feature 2, Feature 3A, Feature 3B, Feature 4, Sprint 4 Feature 1 non-interference.",
			"defer_blocker_type": defer_blocker_type,
			"traceability_evidence_references": ["S4F2-TRACE-001", "S4F2-TRACE-002"],
	...
	def test_go_outcome_and_implementation_authorized_is_zero(self):
		...
		items = evaluate_sprint4_feature2_runtime_safety_non_interference_governance(
	...
	def test_mutation_detection_forces_adr_route_and_no_go(self):
		...
		items = evaluate_sprint4_feature2_runtime_safety_non_interference_governance(
			...
			governance_context={
				...
				"mutation_indicators": {"changes_approval_state_semantics": 1},
			},
		)
		self.assertEqual(items[0].readiness_outcome, NO_GO)
	...
	def test_defer_for_scheduled_governance_activity_when_conditions_pass(self):
		seed = self.seed_dataset()
		package = self.make_evidence_package(
			defer_blocker_type="scheduled_governance_activity",
			defer_blocker_reference="S4F2-SGA-001",
		)
		items = evaluate_sprint4_feature2_runtime_safety_non_interference_governance(
			lighthouse_workflow_charter=seed["charter"].name,
			review_window_start=add_days(today(), -30),
	...
	def test_safety_failures_result_in_no_go(self):
		seed = self.seed_dataset()
		items = evaluate_sprint4_feature2_runtime_safety_non_interference_governance(
			lighthouse_workflow_charter=seed["charter"].name,
			review_window_start=add_days(today(), -30),
	...
	def test_missing_approval_results_in_no_go(self):
		...
		items = evaluate_sprint4_feature2_runtime_safety_non_interference_governance(
			...
			decision_record=seed["decision"].name,
			governance_context={
				"evidence_package": self.make_evidence_package(system_approval="Rejected"),
			},
		)
		self.assertEqual(items[0].readiness_outcome, NO_GO)
	...
	def test_deterministic_behavior(self):
		...
		first = evaluate_sprint4_feature2_runtime_safety_non_interference_governance(
			lighthouse_workflow_charter=seed["charter"].name,
			review_window_start=add_days(today(), -30),
			review_window_end=add_days(today(), 30),
			policy_version="S4F2_POLICY_V1",
		...
		second = evaluate_sprint4_feature2_runtime_safety_non_interference_governance(
			lighthouse_workflow_charter=seed["charter"].name,
			review_window_start=add_days(today(), -30),
			review_window_end=add_days(today(), 30),
			policy_version="S4F2_POLICY_V1",
	...
	def test_actor_trace_audit_evidence_is_recorded(self):
		seed = self.seed_dataset()
		with patch("enterprise_intelligence_platform.sprint4_feature2_runtime_safety_non_interference_governance.frappe.logger") as logger_mock:
			evaluate_sprint4_feature2_runtime_safety_non_interference_governance(
				lighthouse_workflow_charter=seed["charter"].name,
				review_window_start=add_days(today(), -30),
				review_window_end=add_days(today(), 30),
			...
			self.assertEqual(payload["event"], "sprint4_feature2_runtime_safety_non_interference_governance_review_executed")
