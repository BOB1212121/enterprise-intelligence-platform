...
S4F1_AUDIT_LOGGER_NAME = "enterprise_intelligence_platform.sprint4_feature1"
...
@dataclass(frozen=True)
class Sprint4Feature1GovernanceItem:
...
def evaluate_sprint4_feature1_additive_contract_extension_governance(
	*,
	lighthouse_workflow_charter: str,
	review_window_start: str,
	review_window_end: str,
...
) -> tuple[Sprint4Feature1GovernanceItem, ...]:
	...
	items: list[Sprint4Feature1GovernanceItem] = []
...
def _build_item(
	*,
	...
	window_end: date,
	policy_version: str,
	context: Sprint4Feature1GovernanceContext,
) -> Sprint4Feature1GovernanceItem:
	...
	return Sprint4Feature1GovernanceItem(
...
def _build_rationale(
	*,
	...
	parts.append(_("Deterministic Feature 1 governance outcome: {0}. Implementation authorization remains disabled.").format(outcome))
...
def _evaluate_additive_conformance(
	...
	if "sprint 4 feature 1" not in scope_decl:
		return 0, 1
	return 1, 0
...
def _resolve_governance_context(value: dict[str, Any] | None) -> Sprint4Feature1GovernanceContext:
	...
	return Sprint4Feature1GovernanceContext(
...
def _item_sort_key(item: Sprint4Feature1GovernanceItem) -> tuple[Any, ...]:
...
def _record_actor_trace_audit_evidence(
	*,
	...
	policy_version: str,
	context: Sprint4Feature1GovernanceContext,
	items: tuple[Sprint4Feature1GovernanceItem, ...],
) -> None:
	payload = {
		"event": "sprint4_feature1_additive_contract_extension_governance_review_executed",
	...
	frappe.logger(S4F1_AUDIT_LOGGER_NAME).info(payload)
...
def _require_governance_role() -> None:
	...
	if not roles.intersection(GOVERNANCE_ROLES):
		frappe.throw(
			_("Sprint 4 Feature 1 governance review requires a governance role."),
...
__all__ = [
	"Sprint4Feature1GovernanceContext",
	"Sprint4Feature1GovernanceItem",
