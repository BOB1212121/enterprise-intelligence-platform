...
S4F3_AUDIT_LOGGER_NAME = "enterprise_intelligence_platform.sprint4_feature3"
...
class Sprint4Feature3GovernanceContext:
...
@dataclass(frozen=True)
class Sprint4Feature3GovernanceItem:
	...
	rg_009_monitoring_observability_pass: int
	rg_010_traceability_audit_pass: int
	gate_1_pass: int
	gate_2_pass: int
	gate_3_pass: int
	gate_4_pass: int
	...
	def as_dict(self) -> dict[str, Any]:
		return {
			...
			"rg_009_monitoring_observability_pass": self.rg_009_monitoring_observability_pass,
			"rg_010_traceability_audit_pass": self.rg_010_traceability_audit_pass,
...
def evaluate_sprint4_feature3_security_boundary_authority_preservation_governance(
	*,
	lighthouse_workflow_charter: str,
	review_window_start: str,
	review_window_end: str,
...
) -> tuple[Sprint4Feature3GovernanceItem, ...]:
	...
	items: list[Sprint4Feature3GovernanceItem] = []
...
def _build_item(
	*,
	...
	window_end: date,
	policy_version: str,
	context: Sprint4Feature3GovernanceContext,
) -> Sprint4Feature3GovernanceItem:
	...
	trigger_status_invalid = 0
	if trigger_status not in TRIGGER_STATUS_NOT_TRIGGERED | TRIGGER_STATUS_UNRESOLVED | TRIGGER_STATUS_ROUTED:
		trigger_status_invalid = 1
	...
	trigger_routed = 1 if trigger_status in TRIGGER_STATUS_ROUTED else 0

	rg_003 = _evaluate_security_boundary(evidence)
	rg_004, explicit_rejection = _evaluate_approval_authority(evidence)

	rg_005, rg_006, rg_007, rg_008, rg_009 = _evaluate_operational_safety_criteria(
		evidence.get("operational_safety_record")
	)
	rg_010 = _evaluate_traceability_audit(evidence)

	gate_1_pass = 1 if rg_002 == 1 and trigger_unresolved == 0 and trigger_routed == 0 and trigger_status_invalid == 0 else 0
	gate_2_pass = rg_001
	gate_3_pass = _evaluate_gate_3_objective_conformance(evidence, rg_001)
	gate_4_pass = 1 if all(x == 1 for x in (rg_003, rg_004, rg_010)) else 0
	gate_5_pass = 1 if all(x == 1 for x in (rg_005, rg_006, rg_007, rg_008, rg_009)) else 0
	...
	critical_safety_failed = 1 if any(x == 0 for x in (rg_005, rg_006, rg_007, rg_008, rg_009)) else 0
	baseline_or_contract_mutation_trigger = 1 if any(
		x == 1 for x in (mutation_detected, trigger_unresolved, trigger_routed, trigger_status_invalid)
	) else 0
	...
	defer_allowed = 1 if (
		defer_blocker_type in DEFER_BLOCKER_ALLOWED and defer_blocker_ref_reviewable
	) else 0
	...
	return Sprint4Feature3GovernanceItem(
		...
		baseline_change_trigger_status=trigger_status or "not_triggered",
		...
		rg_009_monitoring_observability_pass=rg_009,
		rg_010_traceability_audit_pass=rg_010,
...
def _build_rationale(
	*,
	...
	if trigger_status:
		parts.append(_("Baseline trigger status: {0}.").format(trigger_status))
	...
	parts.append(_("Deterministic Feature 3 governance outcome: {0}. Implementation authorization remains disabled.").format(outcome))
...
def _evaluate_additive_conformance(
	...
	if "sprint 4 feature 3" not in scope_decl:
		return 0, 1
...
def _evaluate_operational_safety_criteria(value: Any) -> tuple[int, int, int, int, int]:
	if isinstance(value, dict):
		...
		return rollback, incident, recovery, operational, monitoring
...
def _resolve_governance_context(value: dict[str, Any] | None) -> Sprint4Feature3GovernanceContext:
	...
	return Sprint4Feature3GovernanceContext(
...
def _item_sort_key(item: Sprint4Feature3GovernanceItem) -> tuple[Any, ...]:
...
def _record_actor_trace_audit_evidence(
	*,
	...
	policy_version: str,
	context: Sprint4Feature3GovernanceContext,
	items: tuple[Sprint4Feature3GovernanceItem, ...],
) -> None:
	payload = {
		"event": "sprint4_feature3_security_boundary_authority_preservation_governance_review_executed",
	...
	frappe.logger(S4F3_AUDIT_LOGGER_NAME).info(payload)
...
def _require_governance_role() -> None:
	...
	if not roles.intersection(GOVERNANCE_ROLES):
		frappe.throw(
			_("Sprint 4 Feature 3 governance review requires a governance role."),
...
__all__ = [
	...
	"GO",
	"NO_GO",
	"Sprint4Feature3GovernanceContext",
	"Sprint4Feature3GovernanceItem",
